...
contract AgeProofZkKYC is Ownable{
    ...
    //a, b, c are the proof
    // input array contains the public parameters: userPubKey[2], isValid, root, currentTime, userAddress, currentYear, currentMonth, currentDay, ageThreshold, investigationInstitutionPubKey[2], encryptedData[2], humanID, dAppID
    function verifyProof(
        ...
        require(input[2] == 1, "the proof output is not valid");

        bytes32 proofRoot = bytes32(input[3]);
        require(KYCRegistry.rootHistory(proofRoot), "the root in the proof doesn't match");
        
        uint proofCurrentTime = input[4];
        ...
        require(tx.origin == address(uint160(input[5])), "transaction submitter is not authorized to use this proof");

        (uint onchainYear, uint onchainMonth, uint onchainDay) = BokkyPooBahsDateTimeLibrary.timestampToDate(onchainTime);

        require(onchainYear == input[6], "the current year is incorrect");
        require(onchainMonth == input[7], "the current month is incorrect");
        require(onchainDay == input[8], "the current day is incorrect");
