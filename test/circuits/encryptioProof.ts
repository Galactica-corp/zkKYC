...
describe('Encryption Proof', () => {
  ...
  it('generates unique shared ECDH key for sender and receiver', async () => {
    ...
    const receiverPriv = BigInt(await getEddsaKeyFromEthSigner(receiver)).toString();
    ...
    const circuitInputs = {
      ...
      "senderPubKey": senderPub.map((p: any) => eddsa.poseidon.F.toObject(p).toString()),
      "receiverPubKey": receiverPub.map((p: any) => eddsa.poseidon.F.toObject(p).toString()),
      "msg": msg,
    }
    const witness = await circuit.calculateLabeledWitness(circuitInputs, sanityCheck);

    const expectedResult = mimcjs.encrypt(msg[0], msg[1], sharedKey[0])

    assert.propertyVal(witness, "main.encryptedMsg[0]", eddsa.poseidon.F.toObject(expectedResult.xL).toString());
    assert.propertyVal(witness, "main.encryptedMsg[1]", eddsa.poseidon.F.toObject(expectedResult.xR).toString());
  });
});
