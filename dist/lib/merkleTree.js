...
class MerkleTree {
    ...
    */
     ...
     */
    calculateEmptyBranchHashes(depth) {
        const levels = [];
    ...
    insertleaves(leaves) {
        ...
        for (let level = 0; level < this.depth; level += 1) {
            ...
            for (let pos = 0; pos < this.tree[level].length; pos += 2) {
                this.tree[level + 1].push(this.calculateNodeHash(this.tree[level][pos], this.tree[level][pos + 1] ?? this.emptyBranchLevels[level]));
            }
        }
    }
    get root() {
